...
if __name__ == '__main__':
    ...
    img1 = cv.imread('m1.png')
    img2 = cv.imread('m2.png')
    ...
    # FlannBasedMatcher是较快的一种特征匹配算法（最近邻搜索）
    flann = cv.FlannBasedMatcher(index_params, search_params) #建立匹配器
    matches = flann.knnMatch(des1, des2, k=2)#得出匹配关键点

    # Need to draw only good matches, so create a mask
    matchesMask = [[0, 0] for i in range(len(matches))] #提取优秀特征点
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        dst_pts = np.float32([kp2[m.trainIdx].pt for m in good]).reshape(-1, 1, 2)#训练模板图像的特征描述子索引
        ...
        warpImg = cv.warpPerspective(testImg, np.array(M), (testImg.shape[1], testImg.shape[0]),
                                     flags=cv.WARP_INVERSE_MAP)#透视变换，新图像可容纳完整的两幅图
